...
export const RiderEdit: React.FC = () => {
    ...
    const mergeRiders = async () => {
        ...
        try {
            const createdAlias = await createAlias(rider, { personId: parseInt(mergePersonId) })
            setAliases((prevAliases) => [...prevAliases, createdAlias]);
            setMergePersonId('');
        } catch (error) {
            showToast({
        ...
        <>
            ...
            <Form
                ...
                formElements={[
                    {
                        label: "First Name",
                        input: <InputText id="firstName" value={rider?.firstname} onChange={(e) => update?.({ firstname: e.target.value })} />
                    },
                    {
                        label: "Last Name",
                        input: <InputText id="lastName" value={rider?.lastname} onChange={(e) => update?.({ lastname: e.target.value })} />
                    },
                    {
                        label: "Email",
                        input: <InputText id="email" value={rider?.email} onChange={(e) => update?.({ email: e.target.value })} />
                    }
                ]}
