...
export const BigScreenPage: React.FC<BigScreenPageProps> = ({
  screenGroupId,
}) => {
  ...
  const screenId =
    params.id ?? urlParams.get("id") ?? localStorage.getItem("bigscreenId");

  useEffect(() => {
    if (screen && screenId && screenId !== screen.id) {
  ...
  useEffect(() => {
    ...
    const onScreenCreated = (screen: BigScreen) => {
      if (screen.id && !urlParams.has("id"))
        localStorage.setItem("bigscreenId", screen.id.toString());
      setScreen(screen);
      document.body.classList.add(screen.role);
      document.documentElement.style.fontSize =
        !screen.rootFontSize || screen.rootFontSize.length === 0
          ? "5vmin"
          : screen.rootFontSize;
    };
    ...
    const onScreenUpdated = (screen: BigScreen) => {
      ...
      if (screenGroup && screenGroup.id !== screen.screenGroup?.id)
        window.location.reload();
      ...
      case "collectingring":
        return (
          <CollectingRingTemplate
            ...
            endTime={
              new Date(templateState.currentTemplate.templateData.endTime)
            }
          />
        );
      case "startlist":
  ...
  return (
    ...
    >
      <div
        className={`template ${templateState.show ? "show" : "hide"} ${
      ...
      {(!screenGroup || screenGroup?.showOsd) && (
        <div className="osd">
