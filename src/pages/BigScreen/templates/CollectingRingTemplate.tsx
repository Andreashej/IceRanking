...
const GroupItem: React.FC<FlatListItem<StartListEntry, Test>> = ({
  item: startListEntry,
  show = true,
  onHidden,
  onShown,
  ...
  return (
    ...
    >
      <div
        className={`row-content ${startListEntry.color}`}
        style={{ gridTemplateColumns: "1fr" }}
      >
        <div
          className="row-marker"
          style={{ backgroundColor: `var(--${startListEntry.color})` }}
        ></div>
        {startListEntry.rider?.firstName} {startListEntry.rider?.lastName}
      </div>
    </li>
  );
};
...
export const CollectingRingTemplate: React.FC<CollectingRingTemplateProps> = ({
  currentGroup,
  test,
  endTime,
}) => {
  ...
  const headerText =
    groupOnCall.length > 0
      ? "Please go to Collecting Ring"
  ...
  return (
    <>
      ...
      <div className="countdown" style={{ gridArea: "timer" }}>
      ...
      <div style={{ gridArea: "group" }}>
        <Header
          headerContent={
            <>
              {test?.testCode} - <small>{headerText}</small>
            </>
          }
