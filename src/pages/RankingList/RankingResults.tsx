...
type CompetitionProps = Required<Pick<Competition, "id" | "name" | "endDate">>;

const RankingResultMarkItem: React.FC<RankingResultMarkProps> = ({
  mark,
  isQualifying,
  ranking,
}) => {
  ...
  const competitionId = mark?.entry?.test?.competitionId;
  ...
  return (
    <>
      {ranking.grouping === "rider" && (
        <Link
          style={{ fontWeight: isQualifying ? "bold" : "normal" }}
          to={`/horse/${mark?.entry?.participant?.equipage?.horseId}/results/${mark?.entry?.test?.catalogCode}`}
        >
          {mark?.entry?.participant?.equipage?.horse?.horseName}
        </Link>
      )}
      {ranking.grouping === "horse" && (
        <Link
          style={{ fontWeight: isQualifying ? "bold" : "normal" }}
          to={`/rider/${mark?.entry?.participant?.equipage?.rider?.personId}/results/${mark?.entry?.test?.catalogCode}`}
        >
          {mark?.entry?.participant?.equipage?.rider?.person?.firstName +
            " " +
            mark?.entry?.participant?.equipage?.rider?.person?.lastName}
        </Link>
      )}
      <Link
        className="d-none d-sm-block"
        style={{ fontWeight: isQualifying ? "bold" : "normal" }}
        to={`/competition/${competition?.id}/test/${mark?.entry?.test?.catalogCode}`}
      >
        {competition?.name}
      </Link>
      <div
        id={`mark-${mark.entryId}-expires-at`}
        className="expires-at d-none d-md-block"
        data-pr-tooltip="Result is no longer valid for this rankinglist after this date."
      >
        <Tooltip target={`#mark-${mark.entryId}-expires-at`} position="top" />
        <span
          style={{
            fontWeight: isQualifying ? "bold" : "normal",
            padding: "0 1rem",
          }}
        >
          <FontAwesomeIcon icon="clock" /> {dateToString(expiresAt, "d/m/Y")}
        </span>
      </div>
      <div
        id={`is-qualifying-${mark.entryId}`}
        className="is-qualifying"
        data-pr-tooltip="This result is used in the calculation of the final mark."
        style={{
          textAlign: "right",
          fontWeight: isQualifying ? "bold" : "normal",
        }}
      >
        {isQualifying && (
          <Tooltip target={`#is-qualifying-${mark.entryId}`} position="top" />
        )}
        {mark.score}
        <FontAwesomeIcon
          icon="check-circle"
          style={{
            marginLeft: ".25rem",
            color: "green",
            visibility: isQualifying ? "visible" : "hidden",
          }}
        />
      </div>
    </>
...
const RankingResultListItem: React.FC<FlatListItem<RankingResult, Ranking>> = ({
  item: result,
  extraData: ranking,
}) => {
  ...
  useEffect(() => {
    if (markRef.current && rankRef.current) {
      setHeight(
        isExpanded
          ? rankRef.current.clientHeight + 20
          : markRef.current.clientHeight
      );
    }
  }, [isExpanded]);
  ...
  const toggle = async () => {
    if (marks.length === 0 && rankingList) {
      ...
      const { promise, cancel } = cancellablePromise(
        getRankingResultMarks(result.id, params)
      );
  ...
  //     const isQualifyingForTestgroup = marks
  //       .filter(
  //         (mark) => mark.test && testgroup.includes(mark.test.catalogCode)
  //       )
  ...
  //   //   const marksForTest = marks.filter(
  //   //     (mark) => mark.test?.catalogCode === testcode
  //   //   );
  ...
  return (
    <li
      ...
      style={{
        gridTemplateColumns: "6ch minmax(0, 1fr) 8ch",
        maxHeight: `${height}px`,
      }}
      ref={ref}
      onClick={toggle}
    >
      <div className="rank" style={{ gridRow: "1 / 4" }} ref={rankRef}>
        {!loading ? (
          ...
          <ProgressSpinner
            style={{ height: "1.5rem", margin: "0.125rem" }}
            strokeWidth="3px"
          />
        )}
      </div>
      ...
      <div className="expanded" style={{ gridColumn: "2 / -1" }}>
        <div className="header-col">
          {ranking.grouping === "horse" ? "Rider" : "Horse"}
        </div>
        <div className="header-col d-none d-sm-block">Competition</div>
        <div
          className="header-col d-none d-md-block"
...
export const RankingResults: React.FC = () => {
  ...
  const getNextPage = useCallback(
    async (rankingId: string): Promise<void> => {
  ...
  const fetchTasks = useCallback(
    async (rankingId) => {
      ...
      const r = (await getRanking(rankingId, params)) as Required<
        Pick<Ranking, "tasksInProgress" | "testgroups">
      >;
    ...
    <>
      <div className="grid-col-2">
        ...
        {user?.superUser && (
          <div
            className="toolbar"
            style={{ width: "max-content", placeSelf: "end" }}
          >
            {toolbar}
          </div>
        )}
