...
const BigScreenRouteRow: React.FC<BigScreenRouteRowProps> = ({
  route: initialRoute,
  screenGroups,
  tests,
  onChange,
  ...
  const searchTests = (event: AutoCompleteCompleteMethodParams) => {
    const existing = route.tests?.map((test) => test.testCode) ?? [];

    const filteredTests = tests.filter(
      (test) =>
        !existing.includes(test.testCode) && test.testCode.includes(event.query)
    );
  ...
  const searchTemplates = (event: AutoCompleteCompleteMethodParams) => {
    const filteredTemplates = TEMPLATES.filter(
      (template) =>
        !route.templates.includes(template) && template.includes(event.query)
    );
...
export const ScreenGroupRoutes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getScreenRoutes(competition, params).then(([screenRoutes]) => {
    ...
    getScreenGroups(sgParams).then(([sg]) => setScreenGroups(sg));
  }, [competition]);
  ...
  return (
    <>
      ...
      <p>
        Routes are applied top to bottom, first match is applied, ordered by
      ...
      <div>
        ...
        {loading && (
          <ProgressSpinner
            style={{ marginLeft: ".5rem", height: "2em", width: "2em" }}
            strokeWidth=".5rem"
          />
        )}
      </div>
