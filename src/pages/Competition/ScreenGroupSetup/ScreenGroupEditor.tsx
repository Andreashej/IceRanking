...
export const ScreenGroupEditor: React.FC<ScreenGroupProps> = ({
  screenGroup,
}) => {
  const [competition] = useCompetition();
  const [osd, setOsd] = useState<boolean>(screenGroup.showOsd);
  const [screens, setScreens] = useState<BigScreen[]>(
    screenGroup.screens ?? []
  );
  ...
  const onDrop: React.DragEventHandler<HTMLDivElement> = (event) => {
    ...
    patchScreen({
      id: draggedElement.dataset.screenId,
      ...
      .catch((err) => {
        ...
        const sourceGroup = document.getElementById(
          `drag-target-${draggedElement.dataset.screenSourceGroup}`
        );
  ...
  const onScreenDelete = (deletedScreenId: string) => {
    setScreens((screens) => {
  ...
  const claim = async (screen: BigScreen): Promise<void> => {
    ...
    try {
      const updatedScreen = await patchScreen({
        id: screen.id,
      ...
      setScreens((prevScreens) => {
        ...
        return [...screensBefore, updatedScreen, ...screensAfter];
      });
  ...
  const createScreen = async (): Promise<void> => {
    try {
      ...
      setScreens((prevScreens) => {
        ...
        return [...screensBefore, newScreen, ...screensAfter];
      });
  ...
  return (
    <>
      <div className="card screen-group-editor">
        ...
        >
          <h3 className="mb-0">
            {screenGroup.name} (ID: {screenGroup.id})
          </h3>
          <div className="d-flex align-items-center">
            <span className="mr-2">Show OSD</span>
            <InputSwitch checked={osd} onChange={(e) => setOsd(e.value)} />
          </div>
        </div>
        <div
        ...
        >
          {screens?.map((screen) => (
            <ScreenEditor
              key={screen.id}
              screenId={screen.id}
              onDelete={onScreenDelete}
            />
          ))}
          <button
            className="screen-editor button add-screen"
            onClick={() => setClaimDialogShown(true)}
          >
      ...
      <Dialog
        header="Claim screens"
        visible={claimDialogShown}
        onHide={() => setClaimDialogShown(false)}
        style={{ width: "50%" }}
      >
        <p>
          Click on one of the screens to add it or click the green plus to add a
          manual screen.
        </p>
        <div
          style={{
