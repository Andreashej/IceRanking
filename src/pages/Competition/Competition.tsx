...
const CompetitionPage: React.FC = ({ children }) => {
  ...
  const subtitle = useMemo<string | null>(() => {
    if (!loading && competition)
      return `${dateToString(competition.startDate, "d/m/Y")} - ${dateToString(
        competition.endDate,
        "d/m/Y"
      )}`;
  ...
  const [menuItems, adminMenuItems] = useMemo<[MenuItem[], MenuItem[]]>(() => {
    ...
    const testItems = competition.tests?.map<MenuItem>((test): MenuItem => {
      const path = `/competition/${competition.id}/test/${test.testCode}`;
    ...
    const adminItems = [
      {
        ...
        className:
          pathname === `/competition/${competition?.id}/edit` ? "active" : "",
        command: () => history.push(`/competition/${competition?.id}/edit`),
      },
      {
        label: "Admins",
        className:
          pathname === `/competition/${competition?.id}/admins` ? "active" : "",
...
export const Competition: React.FC = () => {
  ...
  return (
    <CompetitionProvider competitionId={id}>
      <CompetitionPage>
        <Switch>
          ...
          <Route
            exact
            path="/competition/:id/upload"
            component={CompetitionResultsUpload}
          />
          <Route
            exact
            path="/competition/:id/test/:testcode/edit"
            component={TestEdit}
          />
          <Route
            path="/competition/:id/test/:testcode"
            component={CompetitionResults}
          />
          <Route
            exact
            path="/competition/:id/screengroups"
            component={ScreenGroupSetup}
          />
          <Route
            path="/competition/:id/screengroups/routes"
            component={ScreenGroupRoutes}
          />
