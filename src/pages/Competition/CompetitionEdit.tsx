...
export const CompetitionEdit: React.FC = () => {
  const [competition, updateCompetition, saveCompetition, isChanged] =
    useCompetition();
  const [personSearchTerm, setPersonSearchTerm] = useState(
    competition?.contactPerson?.firstName || ""
  );
    ...
    <>
      ...
      <form id="editCompetition" onSubmit={submit}>
        ...
        <SelectButton
          ...
          onChange={(e) => {
            updateCompetition({ status: e.target.value });
          }}
          className="mb-4"
        />
        <span className="p-float-label">
      ...
      {/* <div>
        <span className="p-float-label mt-4">
          <InputText
            ...
            id="extId"
            value={competition.extId}
          />
          <label className="p-float-label">External ID</label>
          <p>
            <small className="text-muted">
              Used by integrations to other systems, e.g. IcetestNG. Click to
              copy.
            </small>
          </p>
        </span>
      ...
      <EmailDialog
        ...
        initialPerson={competition.contactPerson}
      />
    </>
  );
};
