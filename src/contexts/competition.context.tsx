...
const CompetitionContext = createContext<CompetitionContext | undefined>(
  undefined
);
...
export const CompetitionProvider: React.FC<CompetitionProviderProps> = ({
  competitionId,
  children,
}) => {
  ...
  const saveCompetition: CompetitionContext["save"] = async () => {
    ...
    try {
      const savedCompetition = (await patchCompetition(
        competition
      )) as CompetitionProps;
      setCompetition((prev) => {
        return {
...
export const useCompetition = (): [
  CompetitionContext["resource"]?,
  CompetitionContext["update"]?,
  CompetitionContext["save"]?,
  CompetitionContext["isChanged"]?
] => {
...
export const useTest = (testcode: string): Test | undefined => {
  ...
  const test = context.resource?.tests?.find(
    (test) =>
      (test.catalogCode === testcode && test.catalogCode === test.testCode) ||
