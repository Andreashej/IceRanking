...
import { getAppDetails, getAppDetailsCDN, logInfo } from '../../../utils';
...
export const uploadToAirwatchResign: () => Promise<void> = async () => {
  try {
    const { AD_GROUP } = process.env;
    if (!AD_GROUP) {
      throw new Error('Missing environment variables');
    ...
    logInfo('Getting app details from the .ipa file');
    const { version, bundleId, fileName, appName } = await getAppDetailsCDN();
