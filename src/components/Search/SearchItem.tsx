...
export const SearchItem: React.FC<SearchItemProps> = ({ searchResult }) => {
    ...
    switch (searchResult.type) {
      ...
      case "Person":
        return {
          ...
          primaryText: searchResult.person?.firstName ?? "",
          secondaryText: `${
            searchResult.person?.numberOfResults ?? ""
          } results`,
        };
      case "Horse":
        return {
      ...
      case "Competition":
        return {
          ...
          secondaryText: `${dateToString(
            searchResult.competition?.startDate,
            "d/m/Y"
          )} - ${dateToString(searchResult.competition?.endDate, "d/m/Y")}`,
        };
      default:
        return {
  ...
  return (
    <div className="row px-0 mx-0" style={{ maxWidth: "100%" }}>
      ...
      <div className="col">
        <span className="item-name">{listItem.primaryText}</span>
        <br />
