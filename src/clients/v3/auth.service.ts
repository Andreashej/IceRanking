...
export const login = async (
  ...
  try {
    const response = await apiV2.post<LoginResponse>("/login", {
      username,
      password,
    });
...
export const logout = async (): Promise<void> => {
  try {
    await Promise.all([logoutAccessToken(), logoutRefreshToken()]);
  } catch (error) {
    return Promise.reject(error);
  }
};
...
export const refreshAccessToken = async (): Promise<string> => {
  ...
  try {
    const response = await apiV2.post<Pick<LoginResponse, "accessToken">>(
      "/token/refresh",
      {},
      {
        headers: {
...
export const patchUser = async (user: Partial<User>): Promise<User> => {
  try {
    const response = await apiV2.patch<ApiResponse<User>>(
      `/users/${user.id}`,
      user
    );
...
export const getUser = async (
  ...
  try {
    const response = await apiV2.get<ApiResponse<User>>(`/users/${id}`, {
      params,
    });
